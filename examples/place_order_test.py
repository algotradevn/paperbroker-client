...
try:
    ...
    ord_id = client.place_order("MWG", "BUY", qty=100, price=74500)
    print(f"[STEP 1] Placed order: {ord_id}")

    time.sleep(2)
    status = client.get_order_status(ord_id)
    print(f"[STEP 2] Order status: {status}")

    time.sleep(2)
    client.cancel_order(ord_id)
    print(f"[STEP 3] Cancel sent for order {ord_id}")
